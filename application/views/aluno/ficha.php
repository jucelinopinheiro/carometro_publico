...
<main class="container d-flex justify-content-center flex-wrap p-0">
    <div class="col-md-12 p-2 m-2">
        ...
        <h5>Busca Por: <span id="aluno-matricula" data-url="<?php echo site_url('/');?>"><?php echo (isset($matricula) ? $matricula :'Erro');?></span></h5>
        <hr>
...
<!--MODAL Anexar -->
<div class="modal fade" id="Modal_anexar" tabindex="-1">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">Anexar</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
        <div class="modal-body">
            <div class="form-group row">
                <form id="form-anexar" method="post" enctype="multipart/form-data" action="">
                <label class="col-md-12 col-form-label"><strong>Selecione um arquivo: pdf, png, jpg, ou bmp</strong></label>
                <div class="col-md-10">
                    <input type="file" name="arquivo" id="arquivo">
                </div>
                <input type="hidden" id="idocorrencia" name="ocorrencia">
                <input type="hidden" id="idmatricula" name="matricula">
                
            </div>
        </div>
      <div class="modal-footer">
        <button type="button" id="cancelar-anexar" class="btn btn-secondary" data-dismiss="modal"><i class="fas fa-times"></i> Cancelar</button>
        <button type="button" id="confirma-anexar" class="btn btn-success"> Anexar</button>
        </form>
      </div>
    </div>
  </div>
...
$("#confirma-anexar").click(enviarAnexo);
$("#cancelar-anexar").on('click',function(){
    limpaCampos("#form-anexar");
});

function enviarAnexo(){
    var formData = new FormData($("#form-anexar").get(0));
    $.ajax({
        type : "POST",
        url  : '<?php echo base_url('aluno/anexar_documento');?>',
        data : formData,
        dataType: 'json',
        contentType: false,
        processData:false,
        beforeSend:function(){
            $("#confirma-anexar").prop('disabled', true);
        },
        success: function(resposta){
            $("#confirma-anexar").prop('disabled', false);
...
function alterarFoto(matricula){
    var local =  $("#aluno-matricula").data('url')+'main/alterarfoto/'+matricula;
...
function montaFicha(aluno){
    ...
    var ficha = '<div id="ficha" class="ficha-aluno p-2 card col-xs-12 m-2" data-nome="'+aluno.nome+'" data-matricula="'+aluno.matricula+'">'+
        ...
        '<div class="d-flex bd-highlight mb-1">'+
            '<div class="mr-auto bd-highlight"><button class="btn btn-danger disabled" disabled><i class="fas fa-ghost"></i> Evadir</button></div>'+
            ...
            '<div class="ml-1 bd-highlight"><button class="btn btn-warning"  onclick="alterarFoto('+aluno.matricula+')"><i class="fas fa-portrait"></i> Alterar Foto</button></div>'+
...
function montaOcorrencia(ocorrencia){
    var ficha_ocorrencia = '<div class="ocorrencias ficha-aluno p-2 card col-xs-12 m-2">'+  
        ...
        '</div>'+
        '<div class="d-flex mt-3">'+
            '<p class="m-0 flex-fill"><span class="font-weight-bolder">Anexo: </span><a href="<?php echo base_url('foto/download_anexo/');?>'+ocorrencia.anexo+'">'+ocorrencia.anexo+'</a></p>'+
        '</div>'+
        '<hr>'+
        '<div class="d-flex bd-highlight mb-1">'+
            ...
            '<div class="ml-1 bd-highlight"><button class="btn btn-warning" onclick="anexar('+ocorrencia.id+')"><i class="fas fa-paperclip"></i> Anexar</button></div>'+
